...
export default function UsageGuideModal({ onClose }: UsageGuideModalProps) {
  useEffect(() => {
  ...
  return (
    ...
    >
      <div
        className="relative bg-gray-50 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden"
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-4">
          ...
          {sections.map((section) => {
            ...
            return (
              <div
                key={section.id}
                id={`modal-${section.id}`}
                className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden scroll-mt-4"
              >
                {/* Section header */}
                <div className={`flex items-center gap-3 px-5 py-3.5 ${c.bg} border-b ${c.border}`}>
                  ...
                  {section.steps.map((step, i) => (
                    <div key={i} className="flex gap-3">
                      <div className="flex-shrink-0 mt-0.5">
                        <span className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold ${c.badge}`}>
                          {i + 1}
                        </span>
                      </div>
          ...
          <div className="bg-indigo-50 border border-indigo-200 rounded-2xl px-5 py-4 text-center">
            ...
            <p className="text-xs text-indigo-600 mt-1">
              Puedes navegar entre secciones usando los accesos rápidos en la parte superior.
            </p>
          </div>
        </div>
      </div>
